...
export function AllocateExcessModal({
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={inverse ? "Quitar saldo negativo com excedente disponível" : "Realocar excedente"}
      subtitle="Vincula o EXCEDENTE DO LIMITE DIÁRIO [10+] já realizado a um saldo negativo em aberto — não cria programação futura."
      wide
      footer={
        <>
      ...
      <div className="space-y-4">
        {inverse && selectedDeficit && (
          <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
            <p><span className="font-bold uppercase tracking-wide text-[11px] text-amber-500">{selectedDeficit.originLabel}</span> · {formatDateBR(selectedDeficit.date)}</p>
            <p className="mt-1">Em aberto: <b>{formatMinutes(selectedDeficit.openMinutes)}</b></p>
          </div>
        )}
