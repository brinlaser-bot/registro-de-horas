...
import { FUTURE_DATE_ERROR, formatDateBR, isFutureDate, toMinutes, todayString, type EntryType, type TimeEntryLike } from "@/lib/time";
...
export function CorrectPunchesModal({ open, onClose, date, entries }: Props) {
  ...
  const suggested = time ? suggestedPunchTypeAt(entries, time) : type;
  ...
  return (
    <Modal
      ...
      subtitle={`${formatDateBR(date)} · a posição é o horário, não a ordem de cadastro.`}
      footer={
      ...
      <div className="space-y-4">
        {!analysis.isConsistent ? (
          <p className="rounded-xl border border-amber-300 bg-amber-50 px-3 py-2 text-xs font-medium text-amber-800">
            Registro inconsistente — corrija as batidas para que o dia possa ser calculado.
          </p>
        ) : !analysis.isComplete && entries.length > 0 ? (
          <p className="rounded-xl border border-amber-300 bg-amber-50 px-3 py-2 text-xs font-medium text-amber-800">
            Registro incompleto — há uma entrada sem saída correspondente.
          </p>
        ) : analysis.isComplete ? (
        ...
        <div className="rounded-xl border border-dashed border-slate-300 p-3">
          ...
          <div className="mt-2 flex flex-wrap items-end gap-2">
            <Input label="Horário" type="time" className="w-32" value={time} onChange={(e) => { setTime(e.target.value); setType(suggestedPunchTypeAt(entries, e.target.value)); }} />
            <Select label="Tipo sugerido" className="w-36" value={suggested} onChange={(e) => setType(e.target.value as EntryType)}>
              <option value="entrada">Entrada</option>
              <option value="saida">Saída</option>
            </Select>
            <Button size="sm" loading={busy} onClick={addOne}><Plus size={13} /> Adicionar {suggested === "entrada" ? "entrada" : "saída"}</Button>
          </div>
          {time && (
            <p className="mt-1 text-[11px] text-slate-500">
              Pela posição cronológica, {time} entra como <b>{suggested === "entrada" ? "Entrada" : "Saída"}</b>.
            </p>
          )}
        </div>
