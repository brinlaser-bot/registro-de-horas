...
interface Props {
  ...
  /** Trabalho dentro da janela ABONADO_PARCIAL (alerta visual). */
    ...
    <>
      {workedInAbonoMinutes != null && workedInAbonoMinutes > 0 && (
        <div className="mb-2 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-900">
          ...
          <p>{formatMinutes(workedInAbonoMinutes)} na manhã abonada. Essas horas não quitam a jornada da tarde.</p>
        </div>
      )}
      {compensarHint && (
        <div className="mb-2 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-900">
          ...
          <p className="mt-0.5">
            ...
            {" · "}Trabalhado no próprio dia: <b>{formatMinutes(today.workedMinutes)}</b>
            {" · "}Obrigação efetiva: <b>{formatMinutes(Math.max(0, compensarHint.originalMinutes - today.workedMinutes))}</b>
          </p>
