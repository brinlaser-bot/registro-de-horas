...
export type CalendarTreatment = "ABONADO" | "COMPENSAR" | "ABONADO_PARCIAL";
...
export interface CompanyCalendarEntry {
  ...
  /** HH:MM — início da janela abonada (ABONADO_PARCIAL). Default 08:00. */
  abonoStart?: string;
  /** HH:MM — fim da janela abonada (ABONADO_PARCIAL). Default 12:00. */
  abonoEnd?: string;
}
...
export const CALENDAR_HEADER =
  "data;descricao;categoria;tratamento;horas_a_compensar;jornada_esperada_horas;horas_abonadas;observacao;abono_inicio;abono_fim";
...
function isTime(v: string | undefined): boolean {
...
export function parseCompanyCalendarCsv(text: string, settings: WorkSettings): CalendarImportPreview {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (tratamento !== "ABONADO" && tratamento !== "COMPENSAR" && tratamento !== "ABONADO_PARCIAL") {
      return fail(lineNo, `tratamento não reconhecido: ${tratamento}.`);
    }
    ...
    entries.push({
      ...
      categoria,
      tratamento,
      horasACompensar: tratamento === "ABONADO_PARCIAL" ? 0 : hc,
      jornadaEsperadaHoras: je,
      horasAbonadas: tratamento === "ABONADO_PARCIAL" && (ab ?? 0) === 0 ? 4 : ab,
      observacao: (idx("observacao") >= 0 ? cols[idx("observacao")] : "") || null,
      abonoStart: tratamento === "ABONADO_PARCIAL"
        ? (isTime(idx("abono_inicio") >= 0 ? cols[idx("abono_inicio")] : "") ? cols[idx("abono_inicio")] : "08:00")
        : undefined,
      abonoEnd: tratamento === "ABONADO_PARCIAL"
        ? (isTime(idx("abono_fim") >= 0 ? cols[idx("abono_fim")] : "") ? cols[idx("abono_fim")] : "12:00")
        : undefined,
    });
  }
...
export function statsOf(entries: CompanyCalendarEntry[]) {
  return {
    ...
    abonados: entries.filter((e) => e.tratamento === "ABONADO" || e.tratamento === "ABONADO_PARCIAL").length,
...
function minutesInWindow(
  ...
  return segments.reduce((s, seg) => {
    const x = toMinutes(seg.start);
    const y = toMinutes(seg.end);
    return s + Math.max(0, Math.min(y, b) - Math.max(x, a));
  }, 0);
}
    ...
    const label = marker === "trabalho-feriado"
      ...
      : marker === "feriado"
        ...
        : marker === "abono"
          ...
          : marker === "abono-parcial"
            ? "ABONO PARCIAL — CALENDÁRIO"
          : marker === "recesso"
...
export function abonoDateAdvisory(
  ...
  if (calEntry?.tratamento === "ABONADO_PARCIAL") {
    return "Esta data possui abono parcial do calendário. Recomendamos escolher outra data para aproveitar o Abono de aniversário.";
  }
