...
/** Rótulo da natureza da dívida (nunca colapsar tudo em "déficit"). */
export function debtOriginLabel(kind: CompKind, date?: string, calendars?: CompanyCalendars): string {
  if (kind === "acordo") return "Acordo a compensar";
  if (kind === "deficit") return "Déficit de jornada";
  if (kind === "calendario") {
    const cal = date ? entryOnDate(calendars, date) : undefined;
    if (cal?.categoria === "Recesso Final de Ano") return "Recesso a compensar — Calendário";
    return "Folga a compensar — Calendário";
  }
  return "Excedente";
}

export function realocadoLine(c: Compensation, calendars?: CompanyCalendars): string {
  return `${formatMinutes(c.minutes)} realocados → ${debtOriginLabel(c.kind ?? "deficit", c.sourceDate, calendars)} · ${formatDateBR(c.sourceDate)}`;
}
