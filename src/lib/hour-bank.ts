...
import { compensarObligationOnDate, compensarObligationsInRange, debtOriginLabel } from "./compensar";
import {
  actualExtraForDate,
  allocatedForSource,
  activeAcordos,
...
export function dayCreditView(
  ...
  const finished = !day.empty && !day.open && !day.financialPending;
...
export function hourBankSummary(
  ...
  const negatives = negativeBalanceViews(
    entries, comps, absences, calendars, faltas, settings, range, today,
  );
  const openNegativeTotal = negatives.reduce((s, d) => s + d.openMinutes, 0);
  const openDeficitTotal = negatives.filter((d) => d.kind === "deficit").reduce((s, d) => s + d.openMinutes, 0);
  ...
  return {
    realizedBalance: freeRegularTotal - openNegativeTotal,
    freeRegularTotal,
    openDeficitTotal,
    openNegativeTotal,
...
/** Soma de openMinutes de todas as dívidas já realizadas do ciclo anual. */
export function realizedNegativeOpenMinutes(
  entries: TimeEntry[],
  comps: Compensation[],
  absences: Absence[],
  calendars: CompanyCalendars | undefined,
  ...
  return negativeBalanceViews(entries, comps, absences, calendars, faltas, settings, cycle, today)
...
export interface SpecialExcessLedger {
  ...
  realizedTo: { date: string; minutes: number; portion: "especial" | "regular"; originLabel: string }[];
  ...
  for (const c of related) {
    if (c.status === "concluida") {
      ...
      realizedTo.push({
        ...
        originLabel: debtOriginLabel(kindOf(c), c.sourceDate, calendars),
      });
