...
check("B. substituir o calendário do ciclo não duplica obrigação (e atualiza a derivação)", () => {
  ...
  assert.equal(oblsB.length, 18, "18 obrigações COMPENSAR (Cinzas passou a ABONADO_PARCIAL)");
  assert.equal(oblsB.reduce((s, v) => s + v.originalMinutes, 0), 144 * 60, "144h a compensar no ciclo");
});
...
check("E. 10/02/2027 (Cinzas): ABONADO_PARCIAL — sem obrigação COMPENSAR", () => {
  const cinzas = oblsMain.find((v) => v.date === "2027-02-10");
  assert.equal(cinzas, undefined, "Cinzas deixou de ser COMPENSAR 4h e passou a ABONADO_PARCIAL 4h");
});
