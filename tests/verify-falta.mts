...
check("M. Cinzas 10/02/2027 (ABONO PARCIAL 4h): falta aceita com 4h; déficit 4h; SEM obrigação COMPENSAR", () => {
  const gate = canRegisterFalta("2027-02-10", [], [], both, settings, []);
  assert.equal(gate.ok, true);
  assert.equal(gate.jornadaMinutes, 240, "jornada efetiva = 4h da tarde, nunca 8h");
  reset();
  ...
  assert.equal(deficit?.debtMinutes, 240, "falta efetiva = 4h (jornada da tarde)");
  assert.notEqual(deficit?.debtMinutes, 480, "NUNCA a fórmula paralela de 8h");
  assert.equal(cal, undefined, "Cinzas não gera obrigação COMPENSAR");
});
