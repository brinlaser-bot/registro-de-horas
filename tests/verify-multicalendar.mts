...
check("A. migração companyCalendar → companyCalendars (37 datas/148h/96h preservados)", () => {
  ...
  assert.equal(statsOf(c.entries).totalCompensar, 148 * 60);
  assert.equal(statsOf(c.entries).totalAbonado, 96 * 60);
});
...
check("B. importar 2026–2027 NÃO apaga 2025–2026 (store em memória)", () => {
  ...
  assert.equal(statsOf(cals[0].entries).totalCompensar, 148 * 60);
  assert.equal(statsOf(cals[0].entries).totalAbonado, 96 * 60, "2025–2026 íntegro");
  assert.equal(statsOf(cals[1].entries).totalAbonado, 112 * 60, "2026–2027 = 112h abonadas");
  assert.equal(p2627.stats.compensar, 19, "2026–2027 = 19 obrigações");
  assert.equal(p2627.stats.totalCompensar, 148 * 60, "2026–2027 = 148h");
});
...
check("C/D. companyCalendarForDate e companyDayContext resolvem o ciclo pela data", () => {
  ...
  // 24/12/2026 Abono → 8h; 21/12/2026 Recesso → 8h a compensar; 10/02/2027 Cinzas 4h+4h
  ...
  assert.equal(cinzas.calendarioACompensar, 240);
  assert.equal(cinzas.label, "Compensação parcial — Calendário");
});
...
check("J. backup v3 com dois ciclos: exporta → importa → mantém os dois", () => {
  ...
  if (r.ok) {
    ...
    assert.equal(statsOf(r.backup.companyCalendars![0].entries).totalCompensar, 148 * 60);
    assert.equal(statsOf(r.backup.companyCalendars![1].entries).totalAbonado, 112 * 60);
...
check("O. 30/04→01/05: obrigação não cruza ciclo; cada data usa seu calendário", () => {
  ...
  assert.equal(total, (148 + 148) * 60, "soma das obrigações dos dois ciclos");
});
