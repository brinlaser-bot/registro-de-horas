...
check("B. total a compensar: 148h (8880min) + mensal", () => {
  assert.equal(preview.stats.totalCompensar, 148 * 60);
  assert.equal(statsOf(cal.entries).totalCompensar, 148 * 60);
  ...
  const expected: Record<string, number> = {
    ...
    "2026-01": 16, "2026-02": 12, "2026-03": 0, "2026-04": 16,
  };
  let total = 0;
  ...
  assert.equal(total, 148);
});

/* ── C. Abonadas: 96h derivadas + 19 datas com obrigação ─ */
check("C. abonadas: 96h derivadas + 19 datas com obrigação", () => {
  assert.equal(preview.stats.compensar, 19);
  assert.equal(preview.stats.abonados, 18);
  assert.equal(preview.stats.totalAbonado, 96 * 60);
});
...
check("E. cinzas: esperado 4h + calendário a compensar 4h", () => {
  const v = cctx("2026-02-18");
  assert.equal(v.expectedRegular, 240);
  assert.equal(v.calendarioACompensar, 240);
  assert.equal(v.marker, "calendario-compensar");
  assert.equal(v.label, "Compensação parcial — Calendário");
