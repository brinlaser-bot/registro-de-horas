...
check("6. Restaurar dados de exemplo preserva perfil customizado", () => {
  ...
  assert.ok((restored.excessReasons ?? []).length > 0);
...
check("7. seed continua restaurando os dados operacionais de teste", () => {
  ...
  assert.ok(d.entries.some((e) => e.date === "2026-08-11"));
  assert.ok(d.entries.some((e) => e.date === "2026-08-24"));
  assert.ok(d.entries.filter((e) => e.date === "2026-08-14").length === 6);
  assert.ok(d.absences.some((a) => a.kind === "abono" && a.startDate === "2026-08-10"));
  assert.ok(d.excessReasons?.some((r) => r.date === "2026-08-24"));
